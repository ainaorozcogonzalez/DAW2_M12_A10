...
<body class="bg-gray-50">
        ...
        <aside class="w-64 bg-white shadow-lg">
            <div class="p-6">
                ...
                <nav class="mt-6">
                    ...
                    <a href="{{ route('incidencias.index') }}" class="block py-2 px-4 bg-indigo-50 text-indigo-600 rounded-md">
        ...
        <div class="flex-1">
            ...
            <main class="container mx-auto px-4 py-8">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <button onclick="openIncidenciaModal()" class="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700">
                ...
                <div class="bg-white p-6 rounded-lg shadow-md mb-6">
                    <form action="{{ route('incidencias.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        <div>
                        ...
                        <div>
                            ...
                            <select name="tecnico_id" id="tecnico_id" class="w-full px-3 py-2 border rounded-md">
                                ...
                                @foreach($tecnicos as $tecnico)
                                    <option value="{{ $tecnico->id }}" {{ request('tecnico_id') == $tecnico->id ? 'selected' : '' }}>{{ $tecnico->nombre }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="md:col-span-3 lg:col-span-4 flex justify-end space-x-4">
                            ...
                            <a href="{{ route('incidencias.index') }}" class="bg-gray-300 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-400">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <button onclick="openEditIncidenciaModal({{ $incidencia->id }})" class="text-indigo-600 hover:text-indigo-900 mr-2">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <form action="{{ route('incidencias.destroy', $incidencia) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('¿Estás seguro de eliminar esta incidencia?')">
                ...
                <form class="mt-4 space-y-4" method="POST" id="incidenciaForm">
                    ...
                    <div>
                        <textarea name="descripcion" id="descripcion" placeholder="Descripción"
                            class="w-full px-3 py-2 border rounded-md" autocomplete="off"></textarea>
    ...
    <script>
        ...
        function openIncidenciaModal() {
            ...
            document.getElementById('incidenciaForm').action = "{{ route('incidencias.store') }}";
            resetForm();
